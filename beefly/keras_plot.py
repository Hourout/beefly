...
class PlotMetrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.mode==0:
            ...
            draw(metrics=self.metrics, logs=self.epoch_logs, epoch=epoch, columns=self.columns, iter_num=self.iter_num,
                 mode=self.mode, wait_num=self.wait_num, xlabel=self.xlabel, figsize=self.figsize, cell_size=self.cell_size, valid_fmt=self.valid_fmt)
    ...
    def on_batch_end(self, batch, logs=None):
        if self.mode==1:
            ...
            draw(metrics=self.metrics, logs=self.batch_logs, epoch=self.batch_num, columns=self.columns, iter_num=self.iter_num,
                 mode=self.mode, wait_num=self.wait_num, xlabel=self.xlabel, figsize=self.figsize, cell_size=self.cell_size, valid_fmt=self.valid_fmt)
    ...
    def on_train_end(self, logs=None):
        if self.mode==0:
            draw(metrics=self.metrics, logs=self.epoch_logs, epoch=self.wait_num, columns=self.columns, iter_num=self.iter_num,
                 mode=self.mode, wait_num=self.wait_num, xlabel=self.xlabel, figsize=self.figsize, cell_size=self.cell_size, valid_fmt=self.valid_fmt)
        else:
            draw(metrics=self.metrics, logs=self.batch_logs, epoch=self.wait_num, columns=self.columns, iter_num=self.iter_num,
                 mode=self.mode, wait_num=self.wait_num, xlabel=self.xlabel, figsize=self.figsize, cell_size=self.cell_size, valid_fmt=self.valid_fmt)
